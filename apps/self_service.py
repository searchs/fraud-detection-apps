...
def run_new_report():
    ...
    try:
        for upp_file in request.files.getlist("file"):
            ...
            details = dict()
            details["filesize"] = round(
                float(os.stat(destination).st_size / 1024 / 1024), 2
            )
...
def check():
    return render_template(
        "check.html", title="App Status", env=app.config["ohprice_environment"]
    )
...
def page_not_found(error):
    ...
    return (
        render_template(
            "404.html", message=error, env=app.config["ohprice_environment"]
        ),
        404,
    )
